...
export interface ContainerStatus {
    ...
    state: string;
    status: string;
    ports: string; // e.g. "0.0.0.0:9000->9000/tcp"
}
...
export const getLabStatus = (labPath: string): Promise<ContainerStatus[]> => {
    return new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    for (const line of lines) {
                        ...
                        containers.push({
                            ...
                            ports: data.Publishers ? data.Publishers.map((p: any) => `${p.PublishedPort}->${p.TargetPort}`).join(', ') : (data.Ports || '')
                        });
                    }
                    resolve(containers);
                } catch (e) {
